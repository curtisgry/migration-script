...
// colors used in console log example 'string'.red
const colors = require('colors'); // eslint-disable-line 

const simpleGit = require('simple-git'); // Run git commands from node

simpleGit().clean(simpleGit.CleanOptions.FORCE); // From documentation for simplegit setup

const { Octokit } = require('octokit'); // GitHub api js package for easy requests
...
const repoListTransfer = async (arr) => {
  ...
  for (const i in arr) {      // eslint-disable-line
    await migrateRepo(arr[i]);// eslint-disable-line
  }                           // eslint-disable-line
...
const migrateRepo = async (info) => {
  ...
  while (!completeStatus) {
    ...
    const res = await octokit.request(
      `GET /repos/${ghUsername}/${slug}/import`,
      {
        owner: 'OWNER',
        repo: 'REPO',
      }
...
const inputLoop = async () => {
  ...
  if (isFilteredList === null) {
    ...
    isFilteredList = filterListInput === 'y';
  }
  ...
  if (isRepoList === null) {
    ...
    isRepoList = singleOrListInput.toLowerCase() === 'list';
  }
  ...
  if (isRepoList) {
    runConfirmedInput = prompt(
      'Input GO to confirm and transfer list, press enter to cancel: '
    );
  } else {
    ...
    singleRepo = repoDataList.find(
      (repo) => repo.id === parseInt(repoTransferId)
    );
    console.log(singleRepo);
    runConfirmedInput = prompt(
      'Input GO to confirm and transfer repo, press enter to cancel: '
    );
  }
};
